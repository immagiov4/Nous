...
export const readCollapsedLibraryFolderIds = (
  ...
  try {
    const currentRawFolderIds = storage.getItem(LIBRARY_COLLAPSED_FOLDER_IDS_KEY);
    if (currentRawFolderIds !== null) {
      return parseCollapsedLibraryFolderIds(currentRawFolderIds);
    }
