...
import type { FileData, ProjectSnapshot } from '../../types.ts';
...
const decodeProjectImportPayload = (
  value: unknown,
  invalidMessage = INVALID_BACKUP_FILE_MESSAGE
): DecodedProjectSnapshotWire => {
  try {
    const decoded = decodeProjectSnapshotWire(value);
    if (
      typeof decoded.version !== 'string' ||
      typeof decoded.isLearnMode !== 'boolean' ||
      !Array.isArray(decoded.syllabus) ||
      !Object.hasOwn(decoded, 'learningPlan')
    ) {
...
const inspectArchiveManifest = async (bytes: Uint8Array) => {
  const inspected = await inspectProjectBackupArchive(bytes, PROJECT_ARCHIVE_LIMITS);
  return inspected;
};

export const inspectProjectArchiveData = async (
  file: Blob
): Promise<DecodedProjectSnapshotWire> => {
  const bytes = new Uint8Array(await file.arrayBuffer());
  if (!isZipArchive(bytes)) throw new Error(INVALID_BACKUP_ARCHIVE_MESSAGE);
  const decoded = await decodeProjectBackupArchive(bytes, PROJECT_ARCHIVE_LIMITS);
  return decodeProjectImportPayload(decoded.project, INVALID_BACKUP_ARCHIVE_MESSAGE);
};
...
export const isProjectArchiveFile = async (file: Blob): Promise<boolean> => {
  ...
  try {
    await inspectArchiveManifest(bytes);
    return true;
  } catch {
    return false;
  }
...
export const readLegacyProjectImportData = async (
  file: Blob
): Promise<DecodedProjectSnapshotWire> => {
  const bytes = new Uint8Array(await file.arrayBuffer());
  const parsed = JSON.parse(new TextDecoder().decode(bytes)) as unknown;
  return decodeProjectImportPayload(parsed);
};
