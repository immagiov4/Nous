...
describe('requestGeneratedImage', () => {
  ...
  test('observes a durable image job without a browser-owned timeout', async () => {
    ...
    const result = await requestGeneratedImage('A durable image.', {
    ...
    expect(fetchWithSupabaseAuthMock).toHaveBeenNthCalledWith(
      1,
      'http://localhost:3301/api/generation-jobs/images',
  ...
  test('identifies a terminal raster request timeout', async () => {
    fetchWithSupabaseAuthMock.mockResolvedValue(
      new Response(
        JSON.stringify({
          success: true,
          job: { errorCode: 'generation_timeout', id: 'job-1', status: 'failed' },
        }),
        { status: 200 }
      )
    );
    ...
    await expect(
      requestGeneratedImage('A durable image.', {
        dedupeKey: 'artifact-1',
        projectId: 'project-1',
      })
    ).rejects.toThrow('immagine raster ha superato il tempo disponibile');
  });
