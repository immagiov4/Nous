...
test('readCollapsedLibraryFolderIds treats an empty current value as authoritative', () => {
  const storedValues = new Map<string, string>([
    [LIBRARY_COLLAPSED_FOLDER_IDS_KEY, JSON.stringify([])],
    ['lumina-library-collapsed-folder-ids', JSON.stringify(['folder-legacy'])],
  ]);
  const storage = {
    getItem: (key: string) => storedValues.get(key) ?? null,
  ...
  assert.deepEqual(readCollapsedLibraryFolderIds(storage), []);
});
