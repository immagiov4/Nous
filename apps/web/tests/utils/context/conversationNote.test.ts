...
test('rejects anchors inside code blocks synthesized by the renderer', () => {
  ...
  expect(hasAnchorableConversationNoteCandidate(content, { selectedText: 'while' })).toBe(false);
  assert.equal(
...
test('keeps inline math inside a line classified as renderer-synthesized code', () => {
  ...
  expect(hasAnchorableConversationNoteCandidate(content, { selectedText: 'printf' })).toBe(false);
});
...
test('rejects anchors inside JSON whose missing opening fence is restored by the renderer', () => {
  ...
  expect(hasAnchorableConversationNoteCandidate(content, { selectedText: 'admin' })).toBe(false);
  assert.equal(
