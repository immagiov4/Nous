...
const STYLE = {
  avatarIconSize: 'h-[18px] w-[18px]',
  badgeLabel: 'inline-flex items-center rounded-full px-3 py-1',
  controlIconSize: 'h-4 w-4',
  ctaButton:
    'inline-flex items-center gap-2 rounded-full border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-800',
  frameLg:
    'rounded-[2rem] border border-gray-200 bg-white p-5 shadow-[0_24px_80px_-48px_rgba(24,24,27,0.45)]',
  iconLabel: 'text-sm',
  labContainer: 'min-h-screen bg-[#faf7f2] px-4 py-6 text-gray-900 sm:px-6 lg:px-10',
  labNotice:
    'mb-5 rounded-[1.6rem] border border-gray-200 bg-white px-5 py-4 text-sm leading-6 text-gray-600',
  mobileButton: 'flex w-full items-center gap-3 rounded-[1.1rem] bg-white/65 px-3 py-3 text-left',
  shell: 'overflow-hidden rounded-[1.7rem] border border-black/5',
  shellHeader: 'flex items-center justify-between border-b border-black/8 px-4 py-3',
  textLgCompact: 'text-[2rem]',
  textTiny: 'text-[10px]',
  titleEyebrow: 'text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-400',
  toolbarIconFrame: 'inline-flex h-10 w-10 items-center justify-center',
  variantGrid: 'grid gap-0 border-t border-black/5 lg:grid-cols-[16rem_minmax(0,1fr)]',
  variantIntro: 'mt-5 max-w-[62ch] space-y-4 text-[1.02rem] leading-8 opacity-92',
  variantSectionGrid:
    'mt-8 grid gap-4 border-t border-black/8 pt-4 lg:grid-cols-[minmax(0,1fr)_15rem]',
} as const;
  ...
  return (
    <div className="flex items-center gap-3">
      <span className={`inline-flex h-10 w-10 items-center justify-center ${frameClassName}`}>
        <Icon aria-hidden className={STYLE.avatarIconSize} />
      </span>
...
function PreviewVariantCard({ variant }: { variant: PreviewVariant }) {
  ...
  return (
    <article className={STYLE.frameLg}>
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <p className={STYLE.titleEyebrow}>{variant.label}</p>
        ...
        <span className={`${STYLE.badgeLabel} text-xs font-semibold ${variant.badgeClassName}`}>
          Icon Set
        </span>
      </div>

      <div className={`${STYLE.shell} ${variant.shellClassName}`}>
        <div className={STYLE.shellHeader}>
          <div className="flex items-center gap-3">
            <img src={logoUrl} alt="Nous" className="h-8 w-8 object-contain" />
            <div>
              <p
                className={`${STYLE.textTiny} font-semibold uppercase tracking-[0.2em] opacity-55`}
              >
                Nous Reader
              </p>
          ...
          <div className="flex items-center gap-2">
            {[
              ...
              <span
                ...
                className={`${STYLE.toolbarIconFrame} ${variant.frameClassName}`}
              >
                <Icon aria-hidden className={STYLE.avatarIconSize} />
              </span>
            ))}
        ...
        <div className={STYLE.variantGrid}>
          <aside className="border-r border-black/5 px-4 py-4">
            <p className={`${STYLE.textTiny} font-semibold uppercase tracking-[0.2em] opacity-50`}>
              Sidebar
            </p>
            <div className="mt-4 space-y-3">
              <button type="button" className={STYLE.mobileButton}>
                <span
                  className={`inline-flex h-10 w-10 items-center justify-center ${variant.frameClassName}`}
                >
                  <iconSet.path aria-hidden className={STYLE.avatarIconSize} />
                </span>
          ...
          <section className="px-5 py-5">
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] opacity-55">
              <span className={`${STYLE.badgeLabel} ${variant.badgeClassName}`}>Pausa attiva</span>
              <span className="opacity-60">Lezione 2</span>
            </div>

            <h4
              className={`mt-4 font-serif ${STYLE.textLgCompact} leading-[1.02] tracking-[-0.03em]`}
            >
              Dal messaggio al percorso
            </h4>

            <div className={STYLE.variantIntro}>
              <p>
            ...
            <div className={STYLE.variantSectionGrid}>
              <div>
              ...
              <div className="flex items-end justify-start lg:justify-end">
                <button type="button" className={STYLE.ctaButton}>
                  Prosegui <ChevronRight className={STYLE.controlIconSize} />
                </button>
              </div>
            </div>
          </section>
...
export default function StylePreviewLab() {
  return (
    <main className={STYLE.labContainer}>
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className={STYLE.titleEyebrow}>Temporary Style Lab</p>
        ...
        <div className={STYLE.labNotice}>
          Il confronto adesso è reale: <strong>Lucide</strong>, <strong>Phosphor</strong> e{' '}
          <strong>Heroicons</strong>. Se una famiglia ti convince più delle altre, la posso poi
          applicare davvero alla UI reale nei punti chiave.
        </div>
