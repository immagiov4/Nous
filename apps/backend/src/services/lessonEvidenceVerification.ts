...
Assess only externally checkable claims that materially affect a required lesson objective. Do not assess common terminology, direct explanations of syntax visible in the evidence, or clearly labelled lesson-specific conventions and examples unless their correctness is essential to an objective or safety. Generated visuals are plans awaiting rendering: assess their factual requirements, not whether an unseen rendering implements them. Clip titles are supplied; an absent optional caption is not an unsupported assertion. Do not invent claims about missing output or require artifacts produced by later stages.
