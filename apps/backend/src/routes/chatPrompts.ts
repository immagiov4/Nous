...
- When the user asks to edit an existing artifact, including its title or description, call \`generateCurrentLessonArtifact\` with \`mode: "replacement-draft"\`, the exact \`sourceArtifactId\`, and their changes in \`revisionInstructions\`. Resolve the referenced artifact from previous tool results or \`getCurrentLessonArtifacts\`, which also includes temporary drafts shown in this conversation. Never use new generation for an edit or guess an artifact ID.
