      ...
      <header className="border-b border-gray-200/80 bg-white/90 px-4 py-3 backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-950/90 sm:px-6">
        <div className="mx-auto flex max-w-3xl items-center gap-4">
          ...
          <div className="flex flex-1 items-center gap-3">
            ...
            <span className="text-sm font-medium text-gray-900 dark:text-zinc-100">Calibrazione</span>
          </div>

          <div className="flex items-center gap-3">
            <div className="hidden items-center gap-2 sm:flex">
              <div className="h-1.5 w-24 overflow-hidden rounded-full bg-gray-200 dark:bg-zinc-800">
                <div className="h-full rounded-full bg-orange-500 transition-[width] duration-300" style={{ width: `${progress * 100}%` }} />
              </div>
              <span className="text-xs tabular-nums text-gray-500 dark:text-zinc-500">{Math.round(progress * 100)}%</span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-xs text-gray-600 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400">
              ...
              {tips.map((tip, i) => (
                <span key={tip} className="inline-flex items-center gap-1.5">
                  {i > 0 ? <span className="text-gray-300 dark:text-zinc-700">&middot;</span> : null}
                  {tip}
                </span>
              ))}
            </div>
