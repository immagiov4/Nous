...
const getFenceToken = (line: string): '```' | '~~~' | null => {
  const match = line.match(/^(```|~~~)[^\n]*$/);
  return match ? (match[1] as '```' | '~~~') : null;
};

const parseFencedBlockAt = (
  lines: string[],
  startIndex: number
):
  | {
...
const isTextPseudocodeFence = (block: NonNullable<ReturnType<typeof parseFencedBlockAt>>): boolean =>
...
const isPseudocodeFenceBlock = (
  block: NonNullable<ReturnType<typeof parseFencedBlockAt>>
): boolean => {
...
const mergeSplitTextPseudocodeBlocks = (content: string): string => {
...
const mergeSplitBraceFencedBlocks = (content: string): string => {
  const lines = content.split('\n');
  const output: string[] = [];
...
export const normalizeMarkdownForRendering = (content: string): string => {
  ...
  return mergeSplitTextPseudocodeBlocks(
    mergeSplitBraceFencedBlocks(mergeOrphanedContinuationLinesIntoPreviousFence(parts.join('\n')))
  );
};
