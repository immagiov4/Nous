...
Lesson research uses one Zod contract for the provider response and the durable workflow summary. The provider receives JSON Schema derived from that contract, and the model boundary parses the response again before `research-lesson` returns state. If any workflow step still returns invalid state, the runner persists only the first validation issue code and field path, without the Zod message or rejected value, and uses the same field path as corrective feedback.
