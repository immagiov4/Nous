...
After retrieval and final source classification, `research-lesson` checkpoints the result. The separate `select-lesson-evidence` step selects evidence using the configured lesson model, so corrective selection attempts reuse completed research. The selector inspects complete source units and returns inclusive ranges, supported claims, relevance or omission reasons, and explicit overlap references. Text units retain line boundaries and offsets; transcript units retain canonical segment text and timestamps. Neighboring context is selected for meaning without fixed percentages or quotas. A source can contribute no passages. Drafting receives resolved original excerpts; pedagogical review receives claims and source identities; factual review checks the final pedagogically reviewed draft against the excerpts without rewriting it. Clip intervals must fit within retained transcript passages. Invalid references or unsupported claims produce corrective retry feedback, which is included in the factual verification request on the next attempt. Clip bounds use chronologically ordered copies of retained intervals and their maximum endpoint, preserving imported transcript order and text. Primary-source coverage applies its existing text-length threshold to decoded source text.
...
The current review step persists the validated pedagogical draft through `providerEffect` before factual verification. Malformed factual reports and transport retries reuse that result. A structured corrective-feedback envelope advances the pedagogical revision only when factual claims or clip evidence require content changes; the existing attempt budget still applies. Each new content correction starts from the preceding saved pedagogical revision, preserving earlier repairs. A missing required revision fails explicitly. Model-produced factual findings are delimited as untrusted JSON, following the pedagogical review boundary. Selector feedback stays in input data and cannot replace the fixed provider instructions. Referenced PDF images contribute their stored caption, page and nearby text to factual evidence. This validates textual support, not image pixels. Legacy document loading resolves the primary source ID from the authoritative project reference before constructing excerpts.
