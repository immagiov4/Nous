...
The workspace keeps the frontend generation gate alive when navigation invalidates the current lesson workflow view. If the learner returns to the section that owns an active ordinary-lesson or selection-created sublesson generation, the gate reattaches the view to the existing request instead of starting another generation. Matching ownership takes precedence over cached-content reuse, including force-regeneration re-entry. After a full reload, a stored ordinary-lesson request key also takes precedence over cached content, and the client accepts the backend's active project workflow when its section ID matches the section being reopened. A sublesson keeps its provisional request key until the workflow reaches a terminal state. Once its generated section is recoverable, reopening that section resolves the original run by that request key before considering a new ordinary generation. Only the section ID reported by that run bypasses cached reuse, so another deep-dive under the same parent remains available. A newer navigation request or project switch invalidates a pending retained-request lookup before it can change the visible section. Starting a new deep-dive retires a terminal parent request first, while an active retained request keeps ownership and reports the existing generation as busy. This preserves terminal failures and avoids duplicate provider work even when the project lock has already been released. The project ID, generated section ID and generation token remain authoritative for the provider call and its terminal result. The current project, active section and applicable workflow request ID decide whether progress or an error belongs to the visible view. Detached completion can therefore update its own persisted project independently, and a terminal failure still reaches a current or reattached view without displaying over another project or lesson. Missing-source state follows the same project scope: each affected project retains its own flag, while the reader derives `needsSourceFile` only for the project currently open.

Sources: [apps/backend/src/routes/lessonWorkflows.ts](../../../apps/backend/src/routes/lessonWorkflows.ts), [apps/backend/src/workflows/lessonGenerationApi.ts](../../../apps/backend/src/workflows/lessonGenerationApi.ts), [apps/backend/src/workflows/lessonGenerationStageServices.ts](../../../apps/backend/src/workflows/lessonGenerationStageServices.ts), [apps/backend/src/workflows/retryPolicy.ts](../../../apps/backend/src/workflows/retryPolicy.ts), [apps/backend/src/workflows/postgresWorkflowStepStore.ts](../../../apps/backend/src/workflows/postgresWorkflowStepStore.ts), [apps/web/components/shared/GenerationProgress.tsx](../../../apps/web/components/shared/GenerationProgress.tsx), [apps/web/hooks/workspace/controller/sectionProgression.ts](../../../apps/web/hooks/workspace/controller/sectionProgression.ts), [apps/web/services/openrouter/lessonGenerationClient.ts](../../../apps/web/services/openrouter/lessonGenerationClient.ts)
