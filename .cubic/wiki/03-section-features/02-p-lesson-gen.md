...
The workspace keeps the frontend generation gate alive when navigation invalidates the current lesson workflow view. If the learner returns to the lesson that owns the active generation, the gate reattaches the view to the existing request instead of starting another generation. The project-scoped generation token remains authoritative for the provider call and its terminal result, while the `loadSection` request ID only controls the progress and error state visible for the currently attached project. This prevents another project's reattachment from discarding an earlier generation that settles later. A terminal failure still reaches the current or reattached lesson view, but a detached selection caller settles as stale instead of displaying that failure over another project.
