...
The workspace keeps the frontend generation gate alive when navigation invalidates the current lesson workflow view. If the learner returns to the section that owns an active ordinary-lesson or selection-created sublesson generation, the gate reattaches the view to the existing request instead of starting another generation. Matching ownership takes precedence over cached-content reuse, including force-regeneration re-entry. After a full reload, a stored ordinary-lesson request key also takes precedence over cached content, and the client accepts the backend's active project workflow when its section ID matches the section being reopened. A sublesson keeps its provisional request key until the workflow reaches a terminal state. Once its generated section is recoverable, reopening that section acquires the section request key before clearing the provisional parent key. The project ID, generated section ID and generation token remain authoritative for the provider call and its terminal result. The current project, active section and applicable workflow request ID decide whether progress or an error belongs to the visible view. Detached completion can therefore update its own persisted project independently, and a terminal failure still reaches a current or reattached view without displaying over another project or lesson. Missing-source state follows the same project scope: each affected project retains its own flag, while the reader derives `needsSourceFile` only for the project currently open.
