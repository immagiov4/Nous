...
The tool can resolve the lesson selectors, page through the ordered archive index or one exact directory, search textual entries for a literal string, and read a bounded page from one exact indexed path. Index, directory, and literal-search work return a continuation cursor; each search call reads at most one existing bounded text page and keeps continuation state on the authenticated request, so an archive-wide miss cannot synchronously load and decode every retained file. Exact reads return a byte cursor. JSON-escaped text is shortened at a valid UTF-8 boundary when needed to keep the complete serialized result bounded, and an index entry too large to serialize is reported as omitted while its cursor still advances to later entries. Successful outputs carry the archive name and exact path citations; literal-search citations also carry line and column, including matches that span two search pages. Its result states are intentionally distinct: `no-match` means a completed search or selector resolution found nothing, `unavailable` means the archive is missing or changed, `limit-reached` means either the selector/access layer or serialized contextual retrieval budget is exhausted, and `error` means retrieval failed technically. A generic `searchLibrary` miss is not evidence that retained archive files are absent.
