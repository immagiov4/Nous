...
Production course and lesson workflows call `planResearchSources` before retrieval. The configured `context` model decides whether supplied material is sufficient and returns one selected or skipped decision, with a reason, for each available capability. Web is available through the configured model; YouTube is offered only when its retrieval provider is configured. The planner receives source excerpts or archive-index previews, the topic, the learning context, and assessed lesson coverage gaps. A decision declaring insufficient supplied sources must select at least one capability. Absent supplied material cannot be declared sufficient. Sufficiency describes factual coverage and authority; a video can still be selected for pedagogical value when supplied text is sufficient. Capability selection contains no provider names.
...
A lesson dossier without a new YouTube outcome retains its previous YouTube record, when present, and otherwise omits the optional record. Skipping a channel therefore creates no failure record; actual generation warnings retain their existing persistence path.
