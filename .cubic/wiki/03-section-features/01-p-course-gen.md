...
Markdown/prose integrity, positive definitions, self-sufficiency, ASCII pseudo-visual rejection, active-pause quality and generated-visual planning are always available structural checks. Code and math formatting checks are included when the lesson plan activates the corresponding specialist pack or when the draft contains explicit Markdown code or KaTeX syntax. The draft check looks only for syntax markers. It does not infer subject matter from generated prose. Checks that do not apply return `not-applicable` only where the contract explicitly allows it.
