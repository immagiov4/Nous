...
Markdown/prose integrity, positive definitions, self-sufficiency, ASCII pseudo-visual rejection, active-pause quality and generated-visual planning are always available structural checks. Code and math formatting checks are included only when the lesson plan activates the corresponding specialist pack. Checks that do not apply return `not-applicable` only where the contract explicitly allows it.
