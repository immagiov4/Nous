...
PDF lesson generation follows the same path. The document stage uploads extracted image bytes before it records the provider effect result. Workflow checkpoints and project snapshots retain the `ProjectAssetRef` and image metadata, not the data URL. The byte hash and MIME type identify reusable binary storage. Source ID and source content hash remain in the image metadata, so a revised PDF can reuse unchanged bytes while excluding stale context, captions, and images removed from the source. Older durable metadata without a source hash is refreshed when the project provides the current hash.
