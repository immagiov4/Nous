...
Lesson normalization retains a PDF image only when its `imageRef` is placed in the canonical lesson `contentBlocks` through the shared `PDF_IMAGE` placeholder format. A draft-selected reference is rejected if its Markdown heading anchor is missing or ambiguous. An automatically selected fallback is optional and is omitted when it has no unique structural heading. The normalization stage derives `documentAssets.usedImages` from the retained references, and persistence adopts only the corresponding staged binary assets. This keeps the canonical blocks, derived legacy content, image metadata, and storage lifecycle synchronized.
