...
### Interaction Flow
When a user submits a query, the `HomeChatComposer` sends the message along with current context (selected text, lesson content, or library scope) to the `/api/chat/context` backend endpoint. The backend resolves the appropriate AI model, attaches pedagogical tools, and streams the response back to the UI.
