...
When enabled via the `CODEX_APP_SERVER_ENABLED=true` environment variable, a single internal process serves administrators and assigned users through one operator-controlled Codex account. This architectural choice keeps the app-server private to the backend, while remote users interact with Nous through its standard HTTPS API. It is intended for private, operator-controlled self-hosted instances; hosted or shared deployments must leave it disabled and use OpenAI API or OpenRouter instead.
