...
The initial response uses recorded counters and provider costs without consulting an external service. When costs are missing, the menu requests `?estimate=true` separately. A pending or failed estimate leaves the recorded summary visible. Both requests retain the authenticated account identity; closing the menu or changing account cancels them, including an outstanding provider price lookup.
