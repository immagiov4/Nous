...
The initial response uses recorded counters and provider costs without consulting an external service. Its `hasCostEstimateCandidates` field identifies OpenRouter records without a reported cost. The menu requests `?estimate=true` separately only when that field is true; missing costs from other providers do not cause another aggregation. The same server predicate controls price lookup. A candidate may still lack supported rates or complete billing counters. A pending or failed estimate leaves the recorded summary visible. Both requests retain the authenticated account identity; closing the menu or changing account cancels them, including an outstanding provider price lookup.

Provider-reported USD cost takes precedence, including an explicit zero. Where it is absent, the server can estimate recorded OpenRouter text-token cost from the current model catalogue and complete billing counters. It applies provider long-context brackets per call, separates cached input from regular input and leaves unsupported image output, providers or incomplete cache counters unpriced. The display labels current-rate estimates and missing costs explicitly; the result is not a historical invoice. Catalogue failure leaves reported costs available and estimates unknown.
